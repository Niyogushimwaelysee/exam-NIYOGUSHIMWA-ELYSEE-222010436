...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO meals(MealID, MealName, Description, Price, Category) VALUES (?, ?, ?, ?, ? )");
    $stmt->bind_param("issss", $MealID, $MealName, $Description, $Price, $Category);
    // Set parameters and execute
    $MealID = $_POST['driver_id'];
    $MealName = $_POST['user_id'];
...
<body>
    <center><h2>Meals Table</h2></center>
    <table border="3">
        <tr>
            <th>MealID</th>
            <th>MealName</th>
...
// Prepare SQL query to retrieve all meals
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $MealID = $row['MealID']; // Fetch the MealID
        echo "<tr>
            <td>" . $row['MealID'] . "</td>
            <td>" . $row['MealName'] . "</td>
            ...
            <td><a style='padding:4px' href='delete_meals.php?MealID=$MealID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_meals.php?MealID=$MealID'>Update</a></td> 
        </tr>";
    }
